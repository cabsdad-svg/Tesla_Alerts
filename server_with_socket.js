/**
 * Tesla Alerts Backend - Node.js/Express + SQLite + Socket.IO
 * 
 * A backend service for storing and retrieving crowd-sourced driving alerts
  ...
  `, [], (err, rows) => {
    if (!err && rows.length > 0) {
      socket.emit('initial-stats', {
        totalReports: rows[0].total,
        todayReports: rows[0].today,
        timestamp: new Date().toISOString()
      });
    }
  });
...
const serverInstance = server.listen(PORT, '0.0.0.0', () => {
  console.log(`
  🚗 Tesla Alerts Backend with Socket.IO Started
  ...
  📡 Socket.IO Events:
  - new-report: Emitted when a new report is created
  - report-deleted: Emitted when a report is deleted
  - initial-stats: Sent to newly connected clients
